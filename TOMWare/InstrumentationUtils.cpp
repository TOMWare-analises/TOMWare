...
VOID PauseAtAddress(ADDRINT address) {
    PIN_LockClient();
    std::cout << "[TOMWARE] O TOMWare pausou no endere�o " << std::hex << address << std::endl;
    std::cout << "[TOMWARE] Anexe o depurador ao processo agora." << std::endl;
    std::cout << "[TOMWARE] Pressione Enter para continuar a execucao..." << std::endl;
